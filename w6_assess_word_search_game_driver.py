# The object of the game is for the players to view the board and find words (remember that the words list is
# unknown to the players). Words may be contained in rows (from left to right) or columns (from top to bottom),
# but not backwards. When a player correctly guesses a word that occurs in the words list, that player is 
# awarded points according to a scoring system described in the starter code. The game ends when all words on
# the board that appear in the words list have been guessed.
# The player with the highest score wins.
# The words from the words list and the letters of the board are made up of alphabetic, uppercase characters.


from tkinter.filedialog import askopenfile
import w6_assess_word_search_game
...
def print_board(board):
    ...
    for row_idx in range(len(board)):
        print(w6_assess_word_search_game.make_str_from_row(board, row_idx))
...
def play_game(players, board, words):
    ...
    """
    num_remaining = w6_assess_word_search_game.num_words_on_board(board, words) - len(found_words)
    player_num = 0
    while num_remaining > 0:
        ...
        if w6_assess_word_search_game.is_valid_word(words, guess) and w6_assess_word_search_game.board_contains_word(board, guess) and \
            not guess in found_words:
            w6_assess_word_search_game.update_score(players[player_num % len(players)], guess)
...
words = w6_assess_word_search_game.read_words(words_file)
words_file.close()

# Load the board.
board_file = askopenfile(mode='r', title='Select board file')
board = w6_assess_word_search_game.read_board(board_file)
board_file.close()
